...
class Xv6Test(BuildTest, Test):
   ...
   def run(self):
      ...
      shutil.copy(tester_path, self.project_path + "/user/tester.c")
			...
			#if another C testing file is needed, overwrite echo for it
      if self.tester_helper != "":
         tester_path2 = self.test_path + "/" + self.tester_helper
         self.log("overwritting tester_helper with " + tester_path2)
         shutil.copy(tester_path2, self.project_path + "/user/echo.c")
